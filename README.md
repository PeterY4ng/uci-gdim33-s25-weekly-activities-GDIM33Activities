...
### W10
1. Playtest Setup & Notes

Since the Milestone 2 submission, I have added several core features to the game, including a purple breathing glow effect for coins implemented with an HLSL Shader, a full-screen red screen flash effect when the player takes damage, corresponding sound effects for gun shooting, automatic coin pickup functionality, optimized enemy AI pathfinding logic, and invincibility frames after the player is hit. The test version of the game is available at the Itch.io link: shturl.cc/79Y1oxEQgTNKM399OV8pjc2VdLwb. My core goal for this playtest was to verify the operation of two key features: whether the gun shooting sound effects trigger correctly, and whether the full-screen red screen rendering effect appears properly when the player takes damage. During the test, my deskmate played through the entire game, and all tested features worked perfectly—the shooting sounds were clear, and the red screen effect triggered accurately. The only minor issue was that I forgot to add any control instructions to the Itch.io page. However, my deskmate figured out all core controls, including WASD movement and left-click to shoot, on his own without any tutorial. He also commented that the combat feels smooth and the core "combat-collect" loop is clear and intuitive. I will add complete basic control instructions to the Itch.io page later.

3. Planning Strategy from Group Discussion

Our group summarized a practical game engineering planning strategy based on the system design concepts learned in GDIM 33: first, break down the project before implementation, splitting the entire game into multiple completely independent systems such as the player controller, enemy AI, weapon system, collectible system, and UI feedback system; second, implement and test one system at a time, focusing on only one system per phase and moving on to the next only after confirming the current one is bug-free and stable; finally, use the task step breakdown method, further splitting each large system into smaller executable tasks. This planning method completely changed my perception of project scope. Initially, I thought creating a complete vertical slice was an overwhelming and impossible task, but when I broke it down into small tasks, the entire project became clear and manageable. I could clearly track my progress and identify in advance which parts would require more time.

3. Problem Solved / Improvement Made Today

I completed two main improvements today: first, I added complete basic control instructions to the Itch.io game page, clearly telling players how to move with WASD, shoot with left click, and switch to the melee weapon with right click; second, I optimized the code for the hurt screen flash effect, fixed potential abnormal issues, and ensured that the red screen effect displays stably and correctly when the player takes damage.
